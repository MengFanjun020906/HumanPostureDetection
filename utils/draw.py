...
def draw_trajectory(
    ...
    # 创建标签文件名集合以便快速查找
    label_file_names = {label_file.stem for label_file in label_files}
    ...
    for img_id, image_file in enumerate(progress_bar):
        ...
        # 构造对应的标签文件路径
        label_filename = labels_dir / f"{image_file.stem}.txt"
        
        # 如果存在标签文件，则处理它
        if image_file.stem in label_file_names:
            ...
            # 如果没有标签文件（未检测到篮球），则只绘制轨迹线
            image_filename = str(image_file)
            image = cv2.imread(image_filename, 1)
            
            # 根据enable_trajectory参数决定是否绘制轨迹线
